SampleApp = (function () {
    ...
    function onEnrollUserPressed() {
        ...
        console.log(phoneNumber.value + " " + userPin.value);
        let userImage = "";
        if (userImageList.has(userPin.value)) {
        ...
        getSessionToken(function (sessionToken) {
            ...
            latestProcessor = new EnrollmentProcessor(sessionToken, SampleApp, userImage);
            // console.log("yep!");
            // verifyUser(latestEnrollmentIdentifier);
        });
    }
    
    // getUserImageData().then((d) => console.log(d));
    function getUserImageData() {
        var url = 'http://localhost:8090/api/v1/user/data/1';
        return fetch(url)
